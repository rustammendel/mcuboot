...
from imgtool.main import imgtool, load_key
...
class TestKeygen(TestKeys):
    ...
    @pytest.mark.parametrize("key_type", KEY_TYPES)
    def test_keygen_with_password(self, key_type, tmp_path_persistent):
    ...
    @pytest.mark.parametrize("key_type", KEY_TYPES)
    def test_keygen_with_mismatching_password(self, key_type, tmp_path_persistent):
...
class TestLoading(TestKeys):
    @pytest.mark.parametrize("key_type", KEY_TYPES)
    def test_load_key(self, key_type, tmp_path_persistent):
        gen_key = "./keys/" + key_type + GEN_KEY_EXT

        loaded_key = load_key(gen_key)
        assert loaded_key.key

    @pytest.mark.parametrize("key_type", KEY_TYPES)
    def test_load_key_with_password(self, key_type, tmp_path_persistent, capsys, monkeypatch):
        gen_key = "./keys/" + key_type + "_passwd" + GEN_KEY_EXT

        monkeypatch.setattr('sys.stdin', io.StringIO(self.password))
        loaded_key = load_key(gen_key)
    ...
    def test_load_key_public(self, key_type, encoding, tmp_path_persistent):
        pub_key = "./keys/public/" + key_type + "-" + encoding + PUB_KEY_EXT

        loaded_key = load_key(pub_key)
        assert loaded_key.key

    def test_load_key_non_existent(self, capsys):
        gen_key = "./keys/nonexistent"
